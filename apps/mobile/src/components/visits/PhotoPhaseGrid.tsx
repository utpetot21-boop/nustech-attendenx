...
import type { PhotoCounts } from '@/services/visits.service';
...
interface PhaseSection {
  ...
  photos: Photo[];
  counts: PhotoCounts[keyof PhotoCounts];
}

interface Props {
  sections: PhaseSection[];
  onAddPhoto: (phase: 'before' | 'during' | 'after') => void;
  isUploading?: boolean;
          ...
          >
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <Text style={{ fontSize: 14, fontWeight: '700', color: colorSet.text }}>
                  {PHASE_LABELS[phase]}
                </Text>
                <View
                  style={{
              ...
              <Text
                style={{
                  fontSize: 11,
                  color: filled >= counts.min ? '#34C759' : colorSet.text,
                  fontWeight: '500',
                }}
              >
            ...
            >
              {/* C4: gunakan index dari map — bukan indexOf() yang O(n²) */}
              {photos.map((photo, index) => (
                <TouchableOpacity
                  key={photo.id}
                  onPress={() => setPreviewUri(photo.watermarked_url)}
                ...
                >
                  {isPhaseUploading ? (
                    ...
                    <>
                      ...
                      <Text style={{ fontSize: 10, color: colorSet.text, marginTop: 2 }}>
                        Tambah
                      </Text>
                    </>
                  )}
