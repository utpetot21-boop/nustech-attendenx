    ...
    <View style={{
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1, marginRight: 8 }}>
          <View style={{ width: 38, height: 38, borderRadius: R.sm, backgroundColor: '#FF9500' + '18', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <View style={{ flexDirection: 'row', gap: 8, marginBottom: 10 }}>
        <View style={{ flex: 1, backgroundColor: '#007AFF12', borderRadius: R.sm - 2, padding: 10, borderWidth: B.default, borderColor: '#007AFF25' }}>
          ...
          <Text style={{ fontSize: 12, fontWeight: '600', color: lPrimary(isDark) }}>{fmtDate(item.requester_date)}</Text>
        ...
        <View style={{ flex: 1, backgroundColor: '#34C75912', borderRadius: R.sm - 2, padding: 10, borderWidth: B.default, borderColor: '#34C75925' }}>
          ...
          <Text style={{ fontSize: 12, fontWeight: '600', color: lPrimary(isDark) }}>{fmtDate(item.target_date)}</Text>
          {item.target_shift && (
            <Text style={{ fontSize: 11, color: lTertiary(isDark), marginTop: 2 }}>
      ...
      {item.reject_reason ? (
        <View style={{ backgroundColor: '#FF3B3012', borderRadius: R.sm - 2, padding: 10, borderWidth: B.default, borderColor: '#FF3B3025', marginBottom: 8 }}>
      ...
      {isTarget && item.status === 'pending_target' && (
        <View style={{ flexDirection: 'row', gap: 8, paddingTop: 10, borderTopWidth: B.default, borderTopColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)' }}>
          <TouchableOpacity
            onPress={() => onRespond(item.id, false)}
            style={{ flex: 1, height: 42, borderRadius: R.md, backgroundColor: '#FF3B3014', borderWidth: B.default, borderColor: '#FF3B3030', alignItems: 'center', justifyContent: 'center' }}
          >
            <Text style={{ fontSize: 14, fontWeight: '700', color: '#FF3B30' }}>Tolak</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => onRespond(item.id, true)}
            style={{ flex: 1, height: 42, borderRadius: R.md, backgroundColor: '#34C75914', borderWidth: B.default, borderColor: '#34C75930', alignItems: 'center', justifyContent: 'center' }}
          >
      ...
      {isRequester && ['pending_target', 'pending_admin'].includes(item.status) && (
        <View style={{ paddingTop: 10, borderTopWidth: B.default, borderTopColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)' }}>
          <TouchableOpacity
            onPress={() => onCancel(item.id)}
            style={{ height: 40, borderRadius: R.md, backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : '#F5F5F7', borderWidth: B.default, borderColor: isDark ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.08)', alignItems: 'center', justifyContent: 'center' }}
          >
            <Text style={{ fontSize: 13, fontWeight: '600', color: lSecondary(isDark) }}>Batalkan Permintaan</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
...
function DateRow({ label, date, onPress, isDark }: { label: string; date: Date; onPress: () => void; isDark: boolean }) {
  return (
    <TouchableOpacity
      onPress={onPress}
      style={{
...
export default function ScheduleSwapScreen() {
  ...
  const createMutation = useMutation({
    mutationFn: () => scheduleSwapService.create({
      ...
      target_user_id: swapType === 'with_person' && targetUserId.trim() ? targetUserId.trim() : undefined,
  ...
  function handleRespond(id: string, approved: boolean) {
    const msg = approved ? 'Setujui permintaan tukar jadwal ini?' : 'Tolak permintaan tukar jadwal ini?';
    Alert.alert(approved ? 'Setujui' : 'Tolak', msg, [
      { text: 'Batal', style: 'cancel' },
      { text: approved ? 'Setujui' : 'Tolak', style: approved ? 'default' : 'destructive',
  ...
  const canSubmit = swapType === 'with_own_dayoff' || (swapType === 'with_person' && targetUserId.trim().length > 0);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: pageBg(isDark) }}>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={isDark ? '#FFF' : C.orange} />
        }
      >
        {/* ── Header ── */}
        ...
        <View style={{ paddingHorizontal: 20 }}>
          {isLoading ? (
            ...
            <View style={{ alignItems: 'center', paddingTop: 60 }}>
              <View style={{ width: 64, height: 64, borderRadius: 20, backgroundColor: '#FF950012', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
        ...
        >
          ...
          <ScrollView contentContainerStyle={{ padding: 20 }} keyboardShouldPersistTaps="handled">
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 20 }}>
              {([
                { key: 'with_own_dayoff', label: 'Tukar dengan Hari Libur Saya', icon: 'sunny-outline' as const },
                { key: 'with_person',     label: 'Tukar dengan Rekan',           icon: 'people-outline' as const },
              ] as { key: SwapType; label: string; icon: any }[]).map(({ key, label, icon }) => {
                const active = swapType === key;
                return (
                  <TouchableOpacity
                    key={key}
                    onPress={() => setSwapType(key)}
                    style={{
                      flex: 1, padding: 12, borderRadius: R.md,
                  ...
                  >
                    <Ionicons name={icon} size={20} color={active ? '#FFFFFF' : lSecondary(isDark)} />
                    <Text style={{ fontSize: 12, fontWeight: '700', color: active ? '#FFFFFF' : lSecondary(isDark), textAlign: 'center', lineHeight: 16 }}>
                      {label}
                    </Text>
                  </TouchableOpacity>
                );
            ...
            {/* Tanggal */}
            <Text style={{ fontSize: 12, fontWeight: '700', color: lTertiary(isDark), textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 10 }}>
              Tanggal Jadwal Saya (yang akan dipindah)
            </Text>
            <DateRow label="Tanggal kerja saya" date={requesterDate} isDark={isDark} onPress={() => setPickerFor('requester')} />
            ...
            <Text style={{ fontSize: 12, fontWeight: '700', color: lTertiary(isDark), textTransform: 'uppercase', letterSpacing: 0.8, marginTop: 10, marginBottom: 10 }}>
              {swapType === 'with_own_dayoff' ? 'Hari Libur Saya (yang akan jadi kerja)' : 'Tanggal Jadwal Rekan'}
            </Text>
            <DateRow label={swapType === 'with_own_dayoff' ? 'Hari libur saya' : 'Tanggal rekan'} date={targetDate} isDark={isDark} onPress={() => setPickerFor('target')} />
            ...
            {/* Target User (only for with_person) */}
            {swapType === 'with_person' && (
              <>
                ...
                <TextInput
                  ...
                  placeholder="Masukkan User ID rekan..."
                  ...
                  style={{
                    ...
                    fontSize: 14, color: lPrimary(isDark), marginBottom: 4,
                  }}
                />
                <Text style={{ fontSize: 11, color: lTertiary(isDark), marginBottom: 14 }}>
                  * Hubungi admin atau rekan Anda untuk mendapatkan User ID
                </Text>
              </>
            )}

            {/* Catatan */}
            <Text style={{ fontSize: 12, fontWeight: '700', color: lTertiary(isDark), textTransform: 'uppercase', letterSpacing: 0.8, marginTop: 4, marginBottom: 10 }}>
              Catatan (opsional)
            </Text>
            <TextInput
              value={notes}
