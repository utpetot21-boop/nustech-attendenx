...
type SubTab = 'visits' | 'ba' | 'onhold';

// ── Status config ──────────────────────────────────────────────────────────────
const STATUS_MAP: Record<string, {
  label: string; Icon: React.ElementType;
  bg: string; text: string; dot: string; ring: string; lightBg: string;
}> = {
    ...
    >
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-black/[0.04] dark:border-white/[0.06]">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {v.task_id && (
            <span className="flex items-center gap-1 text-[11px] text-[#AF52DE] bg-[#FAF5FF] px-2 py-0.5 rounded-full">Lihat Tugas</span>
          )}
        </div>
...
// ── Detail Modal (untuk orphan visits tanpa task_id) ──────────────────────────
          ...
          {visit.status === 'completed' && (
            <div className="border-t border-black/[0.06] dark:border-white/[0.08] pt-5">
              <div className="flex items-center justify-between mb-3">
                ...
                {visit.review_status && <ReviewBadge status={visit.review_status} />}
              </div>
              ...
              {visit.review_status ? (
                <div className="space-y-3">
                  <div className="flex items-center gap-3 bg-gray-50 dark:bg-white/[0.04] rounded-xl p-3">
                    ...
                    <span className="text-sm font-semibold text-gray-700 dark:text-white/80">{visit.review_rating}/5</span>
                  </div>
                  {visit.review_notes && (
...
// ── VisitsTab (main export) ────────────────────────────────────────────────────
export function VisitsTab({
  defaultSubTab = 'visits',
  onOpenTask,
}: {
  ...
  const handleVisitClick = (v: Visit) => {
    if (v.task_id) {
      onOpenTask(v.task_id);
    } else {
      setDetail(v);
    }
  };
  ...
  const effectiveStatus = subTab === 'onhold' ? 'on_hold' : (statusFilter || undefined);

  const { data: result, isLoading } = useQuery({
    queryKey: ['admin-visits', subTab === 'onhold' ? 'on_hold' : statusFilter, dateFilter],
    queryFn: () =>
      apiClient.get('/visits', {
        params: { status: effectiveStatus, date: dateFilter || undefined, limit: 100 },
      }).then((r) => r.data),
    enabled: subTab === 'visits' || subTab === 'onhold',
    refetchInterval: 30_000,
  });

  const visits: Visit[] = result?.items ?? [];
  const onHoldVisits = subTab === 'onhold' ? visits : visits.filter((v) => v.status === 'on_hold');
    ...
    <div>
      ...
      <div className="px-4 sm:px-6 mb-4">
        <div className="flex gap-1.5 bg-white dark:bg-white/[0.06] rounded-2xl p-1.5 border border-black/[0.05] dark:border-white/[0.08] w-fit">
          {SUB_TABS.map((t) => (
            <button key={t.key} onClick={() => { setSubTab(t.key); setDetail(null); }}
              ...
              {t.count !== undefined && t.count > 0 && (
                <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded-full ${
                  subTab === t.key ? 'bg-white/20 text-white' : 'bg-[#FF9500]/15 text-[#FF9500]'
                }`}>{t.count}</span>
              )}
            </button>
          ))}
          ...
          isLoading ? <LoadingSpinner /> : visits.length === 0 ? (
            ...
            <>
              <div className="md:hidden space-y-3">
                {visits.map((v) => <VisitCard key={v.id} v={v} onClick={() => handleVisitClick(v)} />)}
              </div>
                        ...
                        <tr key={v.id} className="border-b border-black/[0.03] dark:border-white/[0.04] hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className={`flex items-center gap-1 text-xs font-semibold ${photoCount >= PHOTO_MIN_TOTAL ? 'text-[#166534]' : 'text-[#9A3412]'}`}>
                              <Camera size={12} />{photoCount}/{PHOTO_MIN_TOTAL}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            {v.gps_valid
                          ...
                          <td className="px-4 py-3">
                            <button onClick={() => handleVisitClick(v)}
      ...
      {/* Detail Modal for orphan visits (task_id === null) */}
