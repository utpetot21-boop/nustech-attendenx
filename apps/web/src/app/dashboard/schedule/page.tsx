...
import {
  ...
  RotateCcw, Plus, Pencil, Trash2, CalendarDays, CheckCircle2, X, Zap, Settings,
} from 'lucide-react';
...
export default function SchedulePage() {
  ...
  type SchedTab = 'jadwal' | 'pengaturan';
  const schedTab    = (searchParams.get('tab') ?? 'jadwal') as SchedTab;
  const setSchedTab = (t: SchedTab) => {
    const p = new URLSearchParams(searchParams.toString());
    p.set('tab', t);
    router.push(`/dashboard/schedule?${p.toString()}`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] dark:bg-gray-950">
      ...
      <div className="px-4 sm:px-6 pt-6 pb-4">
        <h1 className="text-[20px] sm:text-[22px] font-bold text-gray-900 dark:text-white tracking-tight">Jadwal</h1>
        <p className="text-[12px] text-gray-400 dark:text-white/40 mt-0.5">
          {schedTab === 'jadwal'
            ? 'Pantau dan generate jadwal karyawan minggu ini'
            : 'Atur konfigurasi jam kerja, tipe shift, dan hari libur'}
        </p>
      </div>
      ...
      <div className="px-4 sm:px-6 mb-5">
        <div className="flex gap-1 p-1 bg-white dark:bg-white/[0.06] rounded-2xl border border-black/[0.05] dark:border-white/[0.08] w-fit">
          {([
            ...
            <button key={key} onClick={() => setSchedTab(key)}
            ...
            <section className="space-y-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 dark:text-white/40">Jadwal karyawan office hours minggu ini</p>
                </div>
              </div>
            ...
            <section className="space-y-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 dark:text-white/40">Jadwal karyawan shift minggu ini</p>
                </div>
              </div>
